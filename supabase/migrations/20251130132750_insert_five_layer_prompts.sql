-- 插入五层架构提示词
-- 根因修复: Edge Function 期望的 key 格式为 {layer}.{category}.{variant}.v{version}
-- 但数据库中只有旧格式的 key (agent.xxx)，导致 PromptRouter 频繁 fallback

-- 1. Core System Layer - 角色定义、核心原则、工具能力、多文件规则
INSERT INTO prompts (key, content, description, category, version, is_active, metadata)
VALUES (
  'core.system.base.v1',
  E'你是一名世界级的**全栈开发工程师兼 UI/UX 设计师**，专注于交付**生产级别 (Production-Ready)、结构清晰、可维护**的 Web 应用。
...
8. **禁止**在 Files 里合并多个文件为一个代码块',
  '核心系统提示词 - 定义 Agent 的基础身份、能力边界和行为准则',
  'system',
  1,
  true,
  '{"layer": "core", "author": "system", "changelog": "初始版本"}'::jsonb
)
...
-- 2. Planner Layer - 任务拆解、文件结构规划
INSERT INTO prompts (key, content, description, category, version, is_active, metadata)
VALUES (
  'planner.web.structure.v1',
  E'## Planner 层：任务规划与结构设计
...
6. 页面组件
7. 根组件和入口文件',
  'Planner 层提示词 - 任务拆解、文件结构规划、依赖分析',
  'task',
  1,
  true,
  '{"layer": "planner", "author": "system", "changelog": "初始版本"}'::jsonb
)
ON CONFLICT (key) DO UPDATE SET
  content = EXCLUDED.content,
  description = EXCLUDED.description,
...
- JS 使用 ES6+ 语法',
  'Coder 层提示词 - 逐文件实现代码，保证完整性和质量',
  'task',
  1,
  true,
  '{"layer": "coder", "author": "system", "changelog": "初始版本"}'::jsonb
)
ON CONFLICT (key) DO UPDATE SET
  content = EXCLUDED.content,
  description = EXCLUDED.description,
...
若发现 File Tree 与实际不一致，**必须先修正输出结构再修代码**。',
  'Reviewer 层提示词 - 质量检查、风格一致性验证、遗漏扫描',
  'task',
  1,
  true,
  '{"layer": "reviewer", "author": "system", "changelog": "初始版本"}'::jsonb
)
ON CONFLICT (key) DO UPDATE SET
  content = EXCLUDED.content,
  description = EXCLUDED.description,
...
-- 5. Debugger Layer - 错误诊断、自我修复
INSERT INTO prompts (key, content, description, category, version, is_active, metadata)
VALUES (
  'debugger.error.diagnosis.v1',
  E'## Debugger 层：错误诊断与自我修复
...
- 迭代统计：[次数、最终状态]
```',
  'Debugger 层提示词 - 错误诊断、假设验证、修复生成',
  'task',
  1,
  true,
  '{"layer": "debugger", "author": "system", "changelog": "初始版本"}'::jsonb
)
ON CONFLICT (key) DO UPDATE SET
  content = EXCLUDED.content,
  description = EXCLUDED.description,
