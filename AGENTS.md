...
- 仓库已接入 Supabase MCP 工具（`apply_migration`、`execute_sql`、`list_tables`、`list_migrations` 等），Agent 在编写或审阅迁移（`supabase/migrations/*.sql`）时，必须先用 MCP 执行 SQL，再用 `list_tables`/`execute_sql` 验证表结构、索引与 RLS 状态，避免手动推测。
- 新增或调整迁移后，请在变更说明中明确写清：使用了哪些 MCP 命令、成功返回值（如 `success: true`）、以及 `list_tables` 或 `execute_sql` 中截取的关键字段，便于审计。
- 若需要发布 Edge Function、创建/删除开发分支、获取项目日志或生成类型定义，同样优先调用 MCP 对应工具，确保所有云端操作可追踪、可重放。
