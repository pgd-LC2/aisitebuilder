...
export function useBuildLogs(options: UseBuildLogsOptions): UseBuildLogsReturn {
  ...
  // 处理新日志
  const handleLogCreated = useCallback((log: BuildLog) => {
    console.log('[useBuildLogs] 新日志:', log.id, log.log_type);
    appendLog(log);
  }, [appendLog]);
  ...
  useEffect(() => {
    ...
    return () => {
      console.log('[useBuildLogs] 清理订阅');
      unsubscribe();
      setIsConnected(false);
    };
  }, [projectId, refreshLogs, handleLogCreated]);
